...
export default function Dashboard() {
  ...
  const [filter, setFilter] = useState("monthly"); // State for the selected filter
  // Update the API queries to include the filter
  const { data: statisticData, isLoading } = useGetStatisticsQuery(filter);
  const { data: analaticsData, isLoading: analaticLoading } = useGetAnalyticsQuery(
    filter,
  );
  const { data: mostEarningdata, isLoading: earningloading } = useGetMostEarningQuery();
  const earningdata = Object.entries(mostEarningdata?.top_months || {}).map(
    ([key, value], index) => ({
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`${statisticData?.data?.orders?.status === 'up' ? "text-green-500" : "text-red-500"} flex items-center text-sm`}>

                    {
                      statisticData?.data?.orders?.status === 'up' ? <svg
                        width="14"
                        height="9"
                        viewBox="0 0 14 9"
                        fill="none"
                    ...
                    <span className="pl-1"> {statisticData?.data?.orders?.growth_percentage}%</span>
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`${statisticData?.data?.total_earning?.status === 'up' ? 'text-green-500' : 'text-red-500'} flex items-center text-sm`}>

                    {
                      statisticData?.data?.total_earning?.status === 'up' ? <svg width="14" height="9" viewBox="0 0 14 9" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`${statisticData?.data?.users?.status === 'up' ? 'text-green-500' : 'text-red-500'} flex items-center text-sm`}>

                    {
                      statisticData?.data?.users?.status === 'up' ? <svg
                        width="14"
                        height="9"
                        viewBox="0 0 14 9"
                        fill="none"
            ...
            <div className="h-[470px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={analaticsData?.data}>
                  ...
                  <XAxis dataKey={"month_name" || "week"} stroke="#999" />
                  <YAxis stroke="#999" />
